...
import {
    ...
    fetchLatestRoutePlans, reviewAllocationPlan, fetchAgentRunHistory,
...
function AdminCommandDashboard() {
    ...
    const loadAll = useCallback(async () => {
        const [campsRes, disastersRes, sitRes, queueRes, plansRes, routesRes] = await Promise.all([
    ...
    const handleReview = async (planId, action) => {
        const result = await reviewAllocationPlan(planId, action);
        if (result.success) {
            setAllocationPlans(prev => prev.filter(p => p.id !== planId));
        } else {
                            ...
                            {allocationPlans.map(plan => (
                                <div key={plan.id} className="border rounded-lg p-2 text-xs">
                                    ...
                                    <div className="flex gap-2 mt-2">
                                        <button onClick={() => handleReview(plan.id, 'approve')} className="flex-1 bg-green-600 text-white rounded py-1">Approve</button>
                                        <button onClick={() => handleReview(plan.id, 'reject')} className="flex-1 bg-gray-200 rounded py-1">Reject</button>
                                    </div>
                                </div>
                            ))}
                        </div>
