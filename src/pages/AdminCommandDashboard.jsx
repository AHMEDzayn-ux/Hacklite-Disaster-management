        ...
        <div className="page-shell">
            ...
            <header className="relative z-10 border-b border-white/10 bg-white/[0.03] animate-fade-in-up">
                <div className="w-full px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between flex-wrap gap-3">
                    ...
                    <button
                        ...
                        disabled={running}
                        className="group px-4 py-2 bg-primary-600 hover:bg-primary-500 hover:-translate-y-0.5 disabled:bg-slate-600 disabled:cursor-not-allowed disabled:hover:translate-y-0 rounded-lg text-sm font-semibold text-white flex items-center gap-2 transition-all duration-150"
                    >
                        {running ? (
                            <>
            ...
            {error && (
                <div className="relative z-10 w-full px-4 sm:px-6 lg:px-8 pt-4">
                    <div className="bg-danger-500/10 border border-danger-400/30 text-danger-300 px-4 py-3 rounded-xl">{error}</div>
                </div>
            )}
            ...
            <main className="relative z-10 mx-auto max-w-[1800px] px-4 py-6 sm:px-6 lg:px-8 grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div className="xl:col-span-2 rounded-xl border border-white/10 bg-white/[0.03] overflow-hidden animate-fade-in-up [animation-delay:60ms]" style={{ height: '600px' }}>
                    <MapContainer
                ...
                <div className="space-y-4 max-h-[600px] overflow-y-auto pr-1 animate-fade-in-up [animation-delay:120ms]">
                        ...
                        <div className="space-y-1 text-xs">
                            {Object.entries(runHistory).map(([name, run]) => (
                                <div key={name} className="flex justify-between items-center">
                                    ...
                                    <span className={run?.status === 'success' ? 'text-success-400' : run?.status === 'failed' ? 'text-danger-400' : 'text-slate-400'}>
                            ...
                            {inFlightPlans.map(plan => {
                                ...
                                return (
                                    <div key={plan.id} className="border border-white/10 bg-white/5 rounded-lg p-2 text-xs">
                                        ...
                                        {route ? (
                                            <p className="text-slate-400 mt-1">🛣️ {route.total_distance_km?.toFixed(1)} km · ~{Math.round(route.total_duration_min)} min</p>
                                        ) : (
                                            <p className="text-slate-400 mt-1 italic">No route generated yet - run the Route Optimization agent.</p>
                                        )}
                    ...
                    <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
                        ...
                        <div className="space-y-2">
                            ...
                            {priorityQueue.length === 0 && <p className="text-xs text-slate-400">No active incidents queued.</p>}
                        </div>
                    </div>
                    ...
                    <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
                        ...
                        <div className="space-y-2">
                            ...
                            {situationReports.length === 0 && <p className="text-xs text-slate-400">Run AI Analysis to generate SITREPs.</p>}
                        </div>
                    </div>

                    <Link to="/admin/inventory" className="block rounded-xl border border-white/10 bg-white/[0.03] p-4 hover:border-white/25 hover:bg-white/[0.08] transition-all text-sm font-semibold text-primary-300">
