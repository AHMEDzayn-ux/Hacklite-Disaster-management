...
const DISTRICT_LOCATIONS = {
  ...
  Batticaloa: [
    { area: "Kallady", lat: 7.7167, lng: 81.7 },
    { area: "Kattankudy", lat: 7.6833, lng: 81.7333 },
    { area: "Eravur", lat: 7.7833, lng: 81.6 },
  ],
  Anuradhapura: [
    { area: "Sacred City", lat: 8.3114, lng: 80.4037 },
    { area: "Mihintale", lat: 8.35, lng: 80.5 },
    { area: "Medawachchiya", lat: 8.5408, lng: 80.4958 },
  ],
  Kurunegala: [
    { area: "Town Center", lat: 7.4867, lng: 80.3647 },
    { area: "Maho", lat: 7.8833, lng: 80.25 },
    { area: "Wariyapola", lat: 7.4833, lng: 80.15 },
  ],
  Ratnapura: [
...
export const generateMissingPersons = async (count = 50) => {
  ...
  for (let i = 0; i < count; i++) {
    const district =
      SRI_LANKA_DISTRICTS[
        Math.floor(Math.random() * SRI_LANKA_DISTRICTS.length)
      ];
    ...
    const person = {
      ...
      additional_info:
        additionalInfoOptions[
          Math.floor(Math.random() * additionalInfoOptions.length)
        ],
...
export const generateDisasters = async (count = 30) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    try {
      ...
      console.log(
        `✓ Created disaster ${i + 1}/${count}: ${type} in ${district}`,
      );
    } catch (error) {
...
export const generateAnimalRescues = async (count = 40) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    try {
      ...
      rescues.push(result);
      console.log(
        `✓ Created animal rescue ${i + 1}/${count}: ${animalType} in ${district}`,
      );
    } catch (error) {
...
export const generateDonations = async (count = 50) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const donation = {
      donor_name:
        Math.random() > 0.3 ? `${firstName} ${lastName}` : "Anonymous",
      donor_email:
        Math.random() > 0.3
    ...
    try {
      ...
      console.log(
        `✓ Created donation ${i + 1}/${count}: ${currency} ${amount}`,
      );
    } catch (error) {
...
export const generateAllTestData = async () => {
  ...
  try {
    ...
    console.log(
      "📊 Total: 50 missing persons, 30 disasters, 40 animal rescues, 20 camps, 50 donations",
    );
