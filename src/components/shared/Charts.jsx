    ...
    return (
        <div className="flex items-center gap-4">
            <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="flex-shrink-0">
                ...
                {centerLabel != null && (
                    <text x="50%" y="48%" textAnchor="middle" fill="#f1f5f9" style={{ fontSize: size * 0.24, fontWeight: 700 }}>{centerLabel}</text>
                )}
                {centerSub && (
                    <text x="50%" y="64%" textAnchor="middle" fill="#94a3b8" style={{ fontSize: size * 0.1 }}>{centerSub}</text>
                )}
            </svg>
    ...
    return (
        <div ref={ref} className="relative w-full">
            <svg width={W} height={height} className="block">
                {grid.map((g, i) => (
                    <g key={i}>
                        <line x1={padL} y1={g.yy} x2={W - padR} y2={g.yy} stroke="rgba(255,255,255,0.08)" strokeWidth={1} />
                        <text x={0} y={g.yy + 3} fill="#94a3b8" style={{ fontSize: 9 }}>{g.v}</text>
                    </g>
                ))}
                {labels.map((l, i) => (i % xStep === 0 || i === n - 1) && (
                    <text key={i} x={x(i)} y={height - 6} textAnchor="middle" fill="#94a3b8" style={{ fontSize: 9 }}>{l}</text>
                ))}
                {series.map((s, si) => {
                ...
                {hover != null && (
                    <line x1={x(hover)} y1={padT} x2={x(hover)} y2={padT + plotH} stroke="rgba(255,255,255,0.25)" strokeWidth={1} strokeDasharray="3 3" />
                )}
