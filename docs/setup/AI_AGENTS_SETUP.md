...
Adds `call-transcription-agent`: emergency call recordings (Sinhala/Tamil/English, code-switching supported) are ingested automatically by the gateway in §9 - there is no admin UI for calls at all, neither upload nor a recordings list; the pipeline runs headless and the reports it produces show up in the normal disaster/missing-person/animal-rescue lists. The audio is transcribed by Gemini, and the resulting transcript is run through the **same extraction module `sms-report` uses for SMS text** (`_shared/reportExtraction.ts`) - a transcript is just treated as another incoming message, so there's one prompt/schema/insert path shared by both channels instead of two that could drift apart. Either way the result lands directly in `disasters`/`missing_persons`/`animal_rescues`.
...
4. **Deploy**: same `--no-verify-jwt` reasoning as the other agents (it accepts either the cron secret from the sweep or a direct per-recording invoke from `receive-call-recording`):
   ```bash
   supabase functions deploy call-transcription-agent --no-verify-jwt
   ```
5. **Cron sweep**: already added as a step in `.github/workflows/ai-agents-schedule.yml`. This is only a safety net - `receive-call-recording` invokes the function directly per-recording for near-real-time results, so the 2-hourly cadence only matters for recovering anything left `pending`/stuck `processing`.
6. **Verify**: place a short test call to the gateway phone (§9), then check the database - `select status, detected_category, error_message from call_recordings order by created_at desc limit 5;` should move `pending` → `processing` → `completed` within roughly the length of the call, and a new row should appear in the matching report table with `reported_via_call = true`.
